---
name: Spring Boot Migration Setup
overview: Create a complete Spring Boot project structure with PostgreSQL, Docker, Testcontainers, Hibernate Envers, Lombok, Flyway, email (MailHog), Spring Security, Thymeleaf, and JWT support. The project will be set up in a new directory alongside the existing NestJS codebase.
todos:
  - id: create-gradle-structure
    content: Create Gradle project structure with build.gradle and settings.gradle including all dependencies (Spring Boot, PostgreSQL, Envers, Lombok, Flyway, Security, JWT, Thymeleaf, Mail, Testcontainers)
    status: completed
  - id: create-application-config
    content: Create application.yml configuration files (base, dev, test profiles) with database, email, and security settings
    status: completed
  - id: create-security-config
    content: Implement Spring Security configuration with JWT authentication filter, token provider, and security filter chain
    status: completed
  - id: create-base-entity
    content: Create BaseEntity class with Hibernate Envers auditing annotations (@Audited, @CreatedDate, @LastModifiedDate)
    status: completed
  - id: create-flyway-migration
    content: Create initial Flyway migration script (V1__Initial_schema.sql) with basic schema structure
    status: completed
  - id: create-docker-files
    content: Create Dockerfile (multi-stage build) and docker-compose.yml with PostgreSQL, MailHog, and Redis services
    status: completed
  - id: create-test-setup
    content: Set up Testcontainers configuration and create example integration test
    status: completed
  - id: create-example-components
    content: Configure Spring Actuator health endpoints and create Application main class to demonstrate the setup works
    status: completed
---
