...
class PeopleImportServiceTest {
    ...
    @InjectMocks
    private PeopleImportService peopleImportService;
    ...
    void testPreviewRows_WithValidRows_ReturnsPreview() {
        ...
        row1.put("displayName", "Person 1");
        rows.add(row1);

        when(authHelper.validateOrg(orgId)).thenReturn(null);

        List<Map<String, Object>> result = peopleImportService.previewRows(orgId, rows);
    ...
    void testPreviewRows_WithNullOrgId_ThrowsBadRequestException() {
        assertThrows(BadRequestException.class, () -> {
            peopleImportService.previewRows(null, new ArrayList<>());
        });
    }
    ...
    void testConfirmImport_WithNullOrgId_ThrowsBadRequestException() {
        assertThrows(BadRequestException.class, () -> {
            peopleImportService.confirmImport(null, new ArrayList<>());
        });
    }
}
